...
public class ProdutoController {
    ...
    public ResponseEntity<String> criarProduto(@Valid @RequestBody ProdutoInput produtoInput) {
        produtoService.criarProduto(produtoInput);
        return ResponseEntity.status(201).body("Produto criado com sucesso");
    }

    @PutMapping("atualizar/{id}")
    public ResponseEntity<String> atualizarProduto(@PathVariable Long id, @Valid @RequestBody ProdutoInput produtoInput) throws Exception {
